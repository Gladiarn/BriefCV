...
import { Activity, FileText, LayoutDashboard, Users, Zap, UserPlus, FilePlus } from "lucide-react";
...
const StatGrid = React.memo(({ statConfig, loading }: { statConfig: any[], loading: boolean }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statConfig.map((stat) => (
          <Card key={stat.id} className="border hover:border-primary flex flex-col justify-between">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card className="col-span-1 md:col-span-2 lg:col-span-2 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold flex items-center gap-2"><Zap className="w-5 h-5 text-pink-500"/> AI Usage (Tokens - {range})</h2>
          </div>
          <div className="flex-1 min-h-[300px]">
            <LazyChart data={stats?.dailyUsage || []} dataKey="tokens" fill="#ec4899" type="area" />
          </div>
        </Card>
        ...
        <Card className="col-span-1 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            <a href="/admin/dashboard/users" className="flex flex-col items-center justify-center p-4 rounded-xl bg-secondary hover:bg-primary/10 hover:text-primary transition-colors text-center cursor-pointer gap-2">
              <Users className="w-6 h-6" />
              <span className="font-bold text-xs">Manage Users</span>
            </a>
            <a href="/admin/dashboard/resumes" className="flex flex-col items-center justify-center p-4 rounded-xl bg-secondary hover:bg-primary/10 hover:text-primary transition-colors text-center cursor-pointer gap-2">
              <FileText className="w-6 h-6" />
              <span className="font-bold text-xs">View Resumes</span>
            </a>
            <a href="/admin/dashboard/settings" className="flex flex-col items-center justify-center p-4 rounded-xl bg-secondary hover:bg-primary/10 hover:text-primary transition-colors text-center cursor-pointer gap-2">
          ...
          <div className="border-t border-border pt-6">
            <h3 className="text-sm font-bold text-muted-foreground mb-4">Calendar</h3>
            <Calendar />
          </div>
        </Card>
      </div>
