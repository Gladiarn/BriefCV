...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      dailyUsage: fillMissingDates(dailyUsage.map(d => ({ date: d._id, tokens: d.tokens })), 'tokens', range),
      dailyUsers: fillMissingDates(dailyUsers.map(d => ({ date: d._id, count: d.count })), 'count', range),
      dailyResumes: fillMissingDates(dailyResumes.map(d => ({ date: d._id, count: d.count })), 'count', range),
      activityCalendar: activityCalendar.map(d => ({ date: d._id, count: d.count })),
      totalUsageCount
    });
